...
public class OSMMap extends JMapViewer {
        ...
        private List<MercTile> tiles_reproj;                                   //List of reprojected tiles in Mercator projection
        ...
        public OSMMap(/*List<Point3DGeographic> reference_points_,*/ EarlyMap early_map_, final ControlPointsForm control_points_form_, boolean[] add_test_point_, boolean[] add_reference_point_, boolean[] enable_add_control_points_,
                ...
                //List of reprojected tiles
                tiles_reproj = new ArrayList<>();
        ...
        public void setControlPointsForm(final ControlPointsForm control_points_form_) {
                control_points_form = control_points_form_;
        }
        ...
        protected void paintComponent(Graphics g) {
                ...
                //Some tiles exist
                if (tiles_reproj != null)
                {
                        
                        //Process all reprojected tiles
                        for (MercTile tile : tiles_reproj) {
                                ...
                                //Get north-west point of the raster
                                Point nw = getMapPosition(new Coordinate(tile.lat_nw, tile.lon_nw), false);

                                //Get south-east point of the raster
                                Point se = getMapPosition(new Coordinate(tile.lat_se, tile.lon_se), false);

                                //Points outside the world bounds, skip drawing
                                if (nw == null || se == null) 
                                        continue;
                                
                                //Find minimum coordinate
                                final int x_min = min(nw.x, se.x), y_min = min(nw.y, se.y);
                                final int x_max = max(nw.x, se.x), y_max = max(nw.y, se.y);

                                //Skip drawing: left/above of the view or right/below of the view
                                if (x_max <= 0 || y_max <= 0 || x_min >= getWidth() || y_min >= getHeight()) {
                                        continue;
                                }
        ...
        public void createOSMGraticule(List<MapPolygonImpl> graticule) {
                ...
                final double lat_step = 5.0;
                final double lon_step = 5.0;
        ...
        {
                //Get reprojected tiles
                return tiles_reproj;
        }
        
        
        public void setReprojectedTiles(final List<MercTile> tiles_reproj_)
        {
                //Set repojected tiles
